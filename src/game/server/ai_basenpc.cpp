...
/////
	...
	{
		Vector forward;
		Vector vecGoal = pRallyPoint->GetAbsOrigin();
		ForceMove( vecGoal, forward, true );
		pRallyPoint = NULL;	// we don't have to recall this ever again, so clear it
	}
...
void CAI_BaseNPC::ForceMove( const Vector &targetPos, const Vector &traceDir, bool bRun ) 
{
	SetEnemy( NULL );
	SetSchedule( SCHED_AMBUSH_MODE );

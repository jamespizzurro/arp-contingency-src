...
#include "contingency_boss_spawner.h"
...
LINK_ENTITY_TO_CLASS( contingency_boss_spawner, CContingencyBossSpawner );
...
void CContingencyBossSpawner::MakeNPC( void )
{
	...
	{
		// Only spawn bosses associated with a Combine wave
		if ( Q_strcmp(NPCClassName, "npc_strider") != 0 )
			return;
	}
	else
		return;

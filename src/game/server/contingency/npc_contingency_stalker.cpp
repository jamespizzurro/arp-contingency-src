...
#include "Sprite.h"
#include "npcevent.h"
#include "npc_contingency_stalker.h"
#include "ai_hull.h"
...
#define MIN_STALKER_FIRE_RANGE		0	// as close as we fucking want!
#define	MAX_STALKER_FIRE_RANGE		99999	// as far as we fucking want!
#define	STALKER_LASER_ATTACHMENT	1
#define	STALKER_SENTENCE_VOLUME		(float)0.35
#define STALKER_LASER_DURATION		3
...
enum 
{
	TASK_STALKER_SCREAM = LAST_SHARED_TASK,
};
	...
	case TASK_RANGE_ATTACK1:
		{
			...
			if ( pEnemy )
			{
				// Always hit our target dead-on (UNFORGIVING!!!)
				m_vLaserTargetPos = GetEnemyLKP() + pEnemy->GetViewOffset();
				m_vLaserDir = m_vLaserTargetPos - LaserStartPosition( GetAbsOrigin() );
	...
	switch ( m_NPCState )
	{
		...
		case NPC_STATE_COMBAT:
		{
			if ( GetEnemy() )
			{
				...
				if( CBaseEntity::FVisible(GetEnemy()) )
...
void CNPC_Stalker::CalcBeamPosition(void)
{
	...
	if ( GetEnemy() )
	{
		...
		float m_fNoiseModX		= 1;
		float m_fNoiseModY		= 1;
		float m_fNoiseModZ		= 1;
...
int CNPC_Stalker::RangeAttack1Conditions( float flDot, float flDist )
{
	...
	// We shouldn't be trying to attack enemies we can't see,
	// because that also means we're not going to be able
	// to hit them with our laser!
...
AI_BEGIN_CUSTOM_NPC( npc_stalker, CNPC_Stalker )
	...
	(
		...
		"	Tasks"
		"		TASK_STOP_MOVING				0"
		"		TASK_SET_FAIL_SCHEDULE			SCHEDULE:SCHED_STALKER_PATROL"

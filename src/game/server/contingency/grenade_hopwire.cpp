...
public:
	...
	// Lethal Stigma
	// ADDED: Hopwire
	// Death messages support for multiplayer!
	//static CGravityVortexController *Create( const Vector &origin, float radius, float strength, float duration );
	static CGravityVortexController *Create( const Vector &origin, float radius, float strength, float duration, CBaseEntity *pOwnerOfHopwireGrenade );

private:

	void	ConsumeEntity( CBaseEntity *pEnt );
	void	PullPlayersInRange( void );
	bool	KillNPCInRange( CAI_BaseNPC *pVictim, IPhysicsObject **pPhysObj );
...
void CGravityVortexController::PullPlayersInRange( void )
{
	...
	if ( dist < 128.0f )
	{
		...
		CTakeDamageInfo deathInfo( GetOwnerEntity(), GetOwnerEntity(), this, 200, DMG_FALL );
...
//-----------------------------------------------------------------------------
bool CGravityVortexController::KillNPCInRange( CAI_BaseNPC *pVictim, IPhysicsObject **pPhysObj )
{
	if ( pVictim && pVictim->CanBecomeRagdoll() )
	{
		// This NPC is valid and can become a ragdoll, so do so!

		CTakeDamageInfo info( this, this, 1.0f, DMG_GENERIC );
		CBaseEntity *pRagdoll = CreateServerRagdoll( pVictim, 0, info, COLLISION_GROUP_INTERACTIVE_DEBRIS, true );
		if ( pRagdoll )
		{
			pRagdoll->SetCollisionBounds( pVictim->CollisionProp()->OBBMins(), pVictim->CollisionProp()->OBBMaxs() );

			CTakeDamageInfo ragdollInfo( GetOwnerEntity(), GetOwnerEntity(), 10000.0f, DMG_GENERIC | DMG_REMOVENORAGDOLL );
			pVictim->TakeDamage( ragdollInfo );

			// Return a pointer to the new ragdoll
...
void CGravityVortexController::PullThink( void )
{
	...
	// Sort through nearby entities, pulling all the ones we can into our vortex!
	CBaseEntity *pEnts[128];
	int numEnts = UTIL_EntitiesInBox( pEnts, ARRAYSIZE(pEnts), mins, maxs, 0 );
	for ( int i = 0; i < numEnts; i++ )
	{
		if ( pEnts[i]->IsPlayer() )
			continue;	// ignore players

		CNPC_Citizen *pCitizen = dynamic_cast<CNPC_Citizen*>( pEnts[i] );
		if ( pCitizen )
			continue;	// ignore support NPCs

		// Added spawnable prop system
		CContingency_SpawnableProp *pSpawnableProp = dynamic_cast<CContingency_SpawnableProp*>( pEnts[i] );
		if ( pSpawnableProp )
			continue;	// ignore spawned props

		CNPC_FloorTurret *pTurret = dynamic_cast<CNPC_FloorTurret*>( pEnts[i] );
		if ( pTurret )
			continue;	// ignore deployed turrets

		IPhysicsObject *pPhysObject = NULL;

		CAI_BaseNPC *pNPC = dynamic_cast<CAI_BaseNPC*>( pEnts[i] );
		if ( pNPC )
			KillNPCInRange( pNPC, &pPhysObject );
		...
		if ( pEnts[i]->VPhysicsGetObject() )
		{
			...
			pEnts[i]->VPhysicsTakeDamage( CTakeDamageInfo( GetOwnerEntity(), GetOwnerEntity(), vecForce, GetAbsOrigin(), m_flStrength, DMG_BLAST ) );
		}
...
// ADDED: Hopwire
// Death messages support for multiplayer!
//CGravityVortexController *CGravityVortexController::Create( const Vector &origin, float radius, float strength, float duration, CBaseEntity *pEntity )
CGravityVortexController *CGravityVortexController::Create( const Vector &origin, float radius, float strength, float duration, CBaseEntity *pOwnerOfHopwireGrenade )
{
	...
	// ADDED: Hopwire
	// Death messages support for multiplayer!
	pVortex->SetOwnerEntity( pOwnerOfHopwireGrenade );
...
void CGrenadeHopwire::Precache( void )
{
	...
	// ADDED: Hopwire
	// Exploding sound!
	PrecacheScriptSound( "Weapon_Hopwire.Explode" );
...
void CGrenadeHopwire::CombatThink( void )
{
	...
	if ( hopwire_vortex.GetBool() )
	{
		// Lethal Stigma
		// ADDED: Hopwire
		// Dramatically lowered radius of vortex to prevent massive slaughter
		// Death messages support for multiplayer!
		//m_hVortexController = CGravityVortexController::Create( GetAbsOrigin(), 512, 150, 3.0f );
		m_hVortexController = CGravityVortexController::Create( GetAbsOrigin(), 200, 150, 3.0f, pPlayer );
...
void CGrenadeHopwire::Detonate( void )
{
	...
	// ADDED: Hopwire
	// Exploding sound!
	EmitSound( "Weapon_Hopwire.Explode" );

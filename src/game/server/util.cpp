...
/////
	...
	// Contingency - James
	// Improve NPCs' perception of their surroundings
...
/////
	...
	// Contingency - James
	// Improve NPCs' perception of their surroundings
	// This function from the AI patch has been redone for Contingency
...
CBasePlayer *UTIL_GetNearestPlayer( const Vector &origin )
{
	...
	CContingency_Player *pPlayer = NULL;
	float flDist = 0.0f;
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		flDist = (pPlayer->GetAbsOrigin() - origin).LengthSqr();
		if ( flDist < nearestDistance )
		{
			pNearestPlayer = pPlayer;
			nearestDistance = flDist;
		}
	}

	// If our last search failed, try the same search but include
	// players who might not be playing
	if ( !pNearestPlayer )
	{
...
CBasePlayer *UTIL_GetNearestVisiblePlayer( CBaseEntity *pLooker, int mask )
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		flDist = (pPlayer->GetAbsOrigin() - pLooker->GetAbsOrigin()).LengthSqr();
		if ( (flDist < nearestDistance) && pLooker->FVisible(pPlayer, mask) )
		{
			pNearestPlayer = pPlayer;
			nearestDistance = flDist;
		}

...
/////
	...
	// Contingency - James
	// Randomly decide whether or not an Overwatch solider is elite
	...
	m_fIsElite = random->RandomInt( 0, 1 );
	const char *pModelName = "models/combine_super_soldier.mdl";
	if ( !IsElite() )
	{
		bool rand = random->RandomInt( 0, 1 );
		if ( rand )
			pModelName = "models/combine_soldier.mdl";
		else
			pModelName = "models/combine_soldier_prisonguard.mdl";
	}

		...
		if ( pPlayerAISquad )
		{
			...
			for ( CAI_BaseNPC *pAllyNpc = pPlayerAISquad->GetFirstMember(&iter); pAllyNpc; pAllyNpc = pPlayerAISquad->GetNextMember(&iter) )
			{
				if ( pAllyNpc->GetCommandGoal() != vec3_invalid )
				{
					...
					pAllyNpc->TargetOrder( UTIL_GetNearestPlayer(pAllyNpc->GetAbsOrigin()), &pAllyNpc, 1 ); // AI Patch Addition.
					if ( !bHadGag )
...
void CNPC_Citizen::PostNPCInit()
{
	...
	{
		if ( ( m_spawnflags & SF_CITIZEN_FOLLOW ) ) // AI Patch Addition.
		{
			m_FollowBehavior.SetFollowTarget( UTIL_GetNearestPlayer(GetAbsOrigin()) ); // AI Patch Addition.
...
void CNPC_Citizen::GatherConditions()
{
	...
	if( IsInPlayerSquad() && hl2_episodic.GetBool() )
	{
		...
		if( IRelationType(UTIL_GetNearestPlayer(GetAbsOrigin())) == D_NU ) // AI Patch Addition.
		{
...
{
	CBasePlayer *pLocalPlayer = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
	...
	case SCHED_ALERT_STAND:
		if( m_NPCState != NPC_STATE_COMBAT && pLocalPlayer && !pLocalPlayer->IsAlive() && CanJoinPlayerSquad() )
		{
			...
			if( flDist < 50 * 12 )
			{
			//AddSpawnFlags( SF_CITIZEN_NOT_COMMANDABLE ); // AI Patch Addition.
	...
	case SCHED_RANGE_ATTACK1:
		...
		if ( !IsMortar( GetEnemy() ) && GetActiveWeapon() && FClassnameIs( GetActiveWeapon(), "weapon_rpg" ) )
		{
			...
			else if ( GetEnemy() ) // AI Patch Addition.
			{
				CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetEnemy()->GetAbsOrigin()); // AI Patch Addition.
				if ( pPlayer && ( ( GetEnemy()->GetAbsOrigin() -  // AI Patch Addition.
...
bool CNPC_Citizen::CanJoinPlayerSquad()
{
	...
	if ( IRelationType( UTIL_GetNearestPlayer(GetAbsOrigin()) ) != D_LI ) // AI Patch Addition.
		return false;
...
{
		if ( IsInPlayerSquad() ) // AI Patch Addition.
	{
		if ( m_FollowBehavior.GetFollowTarget() == UTIL_GetNearestPlayer(GetAbsOrigin()) || // AI Patch Addition.
...
{
	if ( !IsFollowingCommandPoint() || !IsInPlayerSquad() ) // AI Patch Addition.
		return false;

	CHL2_Player *pPlayer = (CHL2_Player *)UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
...
bool CNPC_Citizen::SpeakCommandResponse( AIConcept_t concept, const char *modifiers )
{
	return SpeakIfAllowed( concept, 
						   CFmtStr( "numselected:%d,"
									...
									ShouldSpeakRadio( UTIL_GetNearestPlayer(GetAbsOrigin()) ), // AI Patch Addition.
...
void CNPC_Citizen::CommanderUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	// Under these conditions, citizens will refuse to go with the player.
	// Robin: NPCs should always respond to +USE even if someone else has the semaphore.
	if ( !CanJoinPlayerSquad() ) // AI Patch Addition.
	{
	...
	{
		// Don't say hi after you've been addressed by the player // AI Patch Addition.
...
void CNPC_Citizen::TogglePlayerSquadState()
{
	if ( !IsInPlayerSquad() )
	{
		...
		else if ( m_FollowBehavior.GetFollowTarget() == UTIL_GetNearestPlayer(GetAbsOrigin()) ) // AI Patch Addition.
		{
			SpeakCommandResponse( TLK_STARTFOLLOW );
		}
	}
...
{
	CBasePlayer *pPlayer = UTIL_GetNearestPlayer(GetAbsOrigin()); // AI Patch Addition.
	if ( pPlayer && ( pPlayer->GetAbsOrigin().AsVector2D() - GetAbsOrigin().AsVector2D() ).LengthSqr() < Square(20*12) ) // AI Patch Addition.
...
void CNPC_Citizen::UpdateFollowCommandPoint()
{
	...
	if ( IsInPlayerSquad() )
	{
		...
		else
		{
			...
			if ( m_FollowBehavior.GetFollowTarget() != pNearest ) // AI Patch Addition.
			{
...
{
	TossHealthKit( UTIL_GetNearestPlayer(GetAbsOrigin()), Vector(48.0f, 0.0f, 0.0f)  ); // AI Patch Addition.
}
...
bool CNPC_Citizen::ShouldLookForHealthItem()
{
	...
	CBasePlayer *pNearest = UTIL_GetNearestVisiblePlayer(this); // AI Patch Addition.
	if( pNearest && pNearest->GetHealth() <= pNearest->GetMaxHealth() * 0.75f ) // AI Patch Addition.
		return false;
		...
		for ( int i = 0; i < MAX_CITIZEN_RESPONSES; i++ )
		{
			if ( m_flResponseAddedTime[i] )
			{
				...
				else if ( m_flNextResponseTime < gpGlobals->curtime )
				{
					...
					{
					CBasePlayer *pPlayer = UTIL_GetNearestPlayer(pCitizen->GetAbsOrigin()); // AI Patch Addition.

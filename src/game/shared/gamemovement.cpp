...
bool CGameMovement::CheckJumpButton( void )
{
	...
	if ( g_bMovementOptimizations )
	{
		...
		flMul = sqrt(2 * sv_gravity.GetFloat() * GAMEMOVEMENT_JUMP_HEIGHT); // Modification Movement Addition: so jump height is determined by settings in gamemovement.h jump height/time.
#else

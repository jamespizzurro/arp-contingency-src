...
/////
	...
	C_Contingency_Player *pContingencyPlayer = C_Contingency_Player::GetLocalContingencyPlayer();
	if ( pContingencyPlayer )
	{
		float relativeHealth = 0.0f;;
		if ( !pContingencyPlayer->IsObserver() )	// observers are spared (players who just died are not though!)
			relativeHealth = 1.0f - ((float)pContingencyPlayer->GetHealth() / (float)pContingencyPlayer->GetMaxHealth());

		Q_memcpy( &clCallback_data.player_health_ratio, &relativeHealth, sizeof(float) * 1 );
	}
...
pFnClCallback_Declare( ClCallback_PlayerHealthRatio )
{
	m_Lock.Lock();
	Q_memcpy( pfl4, &clCallback_data.player_health_ratio, sizeof(float) * 1 );
	m_Lock.Unlock();
}
...
/////
	...
	shaderEdit->RegisterClientCallback( "local player health ratio", ClCallback_PlayerHealthRatio, 1 );
